...
function expected(r){return Math.round((+r.sales||0)*((+r.rate||0)/100));}
...
function render(){var r=root();if(!r)return;injectCss();
  ...
  var rows=ROWS.filter(function(x){return !flt||statusOf(x)===flt;}).map(function(x){var st=statusOf(x);
    return '<tr><td class="mono">'+esc(x.id)+'</td><td><strong>'+esc(x.source)+'</strong></td><td>'+esc(x.type)+'</td><td>'+esc(x.period)+'</td>'
      +'<td>'+money(x.sales)+'</td><td>'+esc(x.rate)+'%</td>'
      +'<td><strong>'+money(expected(x))+'</strong></td><td style="color:#16a34a">'+money(x.received)+'</td>'
  ...
  }).join('')||'<tr><td colspan="11" style="text-align:center;color:var(--text3);padding:24px">No commission rows.</td></tr>';
  ...
  r.innerHTML=''
   ...
   +'<div class="tvc-card tvc-pad"><div class="tvc-h">➕ Add Commission Rule / Entry</div><div class="tvc-add">'
     ...
     +'<input id="tvc-rate" type="number" step="0.1" placeholder="Rate %" style="width:90px">'
   ...
   +'<div class="tvc-card tvc-pad"><div class="tvc-h">💼 Commission Statement <span class="tvc-chips">'+chip('','All')+chip('Pending','Pending')+chip('Partial','Partial')+chip('Received','Received')+'</span></div>'
     +'<table class="tvc-tbl"><thead><tr><th>ID</th><th>Source</th><th>Type</th><th>Period</th><th>Sales Volume</th><th>Rate</th><th>Expected</th><th>Received</th><th>Outstanding</th><th>Status</th><th style="text-align:right">Action</th></tr></thead><tbody>'+rows+'</tbody></table></div>';
}
...
window.tvcAdd=function(){var g=function(id){var e=document.getElementById(id);return e?e.value:'';};
  ...
  ROWS.unshift({id:uid(),source:g('tvc-src'),type:g('tvc-type'),period:g('tvc-period').trim()||'—',sales:sales,rate:rate,received:0});save();render();};
